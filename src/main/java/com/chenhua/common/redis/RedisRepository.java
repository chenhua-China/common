...
public class RedisRepository {
        ...
        (RedisCallback<Long>)
            connection -> {
              ...
              for (int i = 0; i < keys.length; i++) {
                ...
                // LOGGER.info("[redisTemplate redis]放入 缓存 url:{} ========缓存时间为:{}秒", keys[i],
                // time);
              }
              return 1L;
            });
  ...
  public byte[] get(final byte[] key) {
    byte[] result =
        redisTemplate.execute((RedisCallback<byte[]>) connection -> connection.get(key));
    ...
    return redisTemplate.execute(
        (RedisCallback<Map<String, String>>)
            connection -> {
              ...
              Map<String, String> maps = new HashMap<>(keys == null ? 0 : keys.size());
              for (String key : keys) {
  ...
  public void putHashValue(String key, String hashKey, String hashValue) {
    /*
     * LOGGER.
     * info("[redisTemplate redis]  putHashValue()  key={},hashKey={},hashValue={} "
  ...
  public Object getHashValues(String key, String hashKey) {
    // LOGGER.info("[redisTemplate redis] getHashValues() key={},hashKey={}", key,
    // hashKey);
